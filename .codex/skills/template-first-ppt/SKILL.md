...
## Polish copy before fitting it into modules

Use the separate `slide-copy-polisher` skill before capacity validation when the task includes slide copy, speaker notes, or a text-heavy candidate. Bind every text slot to its role first: main title, module title, body, conclusion, caption, or note. Write the judgment first, then the evidence, explanation, or action. Remove defensive preambles, indirect argument, substantive repetition, empty qualifiers, and unnecessarily complex sentences. Do not use em dashes. Split long sentences with full stops, commas, colons, or semicolons.

The polisher may change wording and notes only. It must preserve facts, numbers, terminology, citations, conclusion strength, fixed title lines, mixed-color run roles, and capacity contracts. If the copy does not fit, rewrite it or choose a compatible module. Never truncate automatically, add filler words, shrink text, enlarge a text box, or change the module geometry.
...
4. Present every candidate as a finished sample slide, not a wireframe or template instruction page. Fill it with substantive example copy and real evidence. Run the direct-copy polish before the capacity and render checks. Keep extension notes, replaceable-field descriptions, sibling variants, and phrases such as “适用” or “迁移方式” in the internal review record, not on the visible slide.
...
1. Turn the request into a slide-by-slide narrative and define each page's communication job, text roles, copy, and real-image need. Run the direct-copy polish before choosing final wording for capacity slots.
...
8. Replace only existing text and pictures. Honor capacity contracts, explicit title lines, and original run-style roles. Use real traceable images with accurate captions and a task-local source manifest. Inventory every semantic content picture in the final deck, including evidence images retained inside a module, not only frames actively replaced. Enforce zero content-image reuse across the entire deck: bind one independent original asset to each content-image frame. Treat copies, renames, recompression, format conversion, crop variants, flips, color edits, screenshots, and a paper page plus a crop from that same page as reuse rather than new material. Exclude only fixed brand imagery explicitly listed by the family contract. Do not write `shared_visual_slot` or any other field that collapses multiple frames into one visual record.
9. Audit image uniqueness before replacement and again against the final source manifest. Require unique normalized paths, source-file SHA256 values, perceptual hashes, stable source identities, and final embedded bytes; review every perceptual near-match. New manifests should include `source_id` or a direct original-asset URL. Run `python script/audit_ppt_image_uniqueness.py --sources <sources.json> --repo-root <repo> --expected-count <picture-frame-count> --output <internal-audit.json>` and then `python script/verify_ppt_image_sources.py --input <working.pptx> --sources <sources.json>`. Any unexplained duplicate is a hard failure. There is no same-image comparison exception for content images.
10. Run `verify-module` for every selected module, `python script/template_ppt.py verify-layered --input <working.pptx>`, and `verify-master` for the complete deck. Run the prose audit from `slide-copy-polisher` and confirm that the final rendered text still has no defensive preambles, indirect argument, substantive repetition, unresolved long sentences, or em dashes. Any master mismatch, unapproved layout, shell drift, module geometry drift, stale provenance, failed image-uniqueness audit, or failed prose audit is a failure.
...
- Do not reuse one content image in two frames or pages by copying, renaming, re-encoding, cropping, flipping, recoloring, or extracting a crop from a document page already used elsewhere. Never use `shared_visual_slot` or an equivalent exception. Fixed non-evidentiary master-family branding is outside this content-image rule; semantic photographs, screenshots, diagrams, and illustrations inside modules are not.
- Do not ship copy with defensive preambles, indirect argument, substantive repetition, unresolved complex sentences, or em dashes. Use `slide-copy-polisher` and resolve its audit findings before fitting text into the module.
